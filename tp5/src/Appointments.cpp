...
Appointments::Appointments(std::string filePath) : _csvHandler(CSVHandler(filePath))
{
    ...
    this->_data=std::vector<std::vector<std::string>>(); // initialisatio
...
void Appointments::displayHeader(std::vector<unsigned int>& displayedColumns)
{
...
void Appointments::filterAppointments(unsigned int userCol, std::string userId, std::vector<std::vector<std::string>>& filteredAppointments)
{
...
bool Appointments::schedule(std::string patientId, std::string nurseId)
{
    ...
    for (std::vector<std::string> appointment : *(getData()))
    {
    ...
    getData()->push_back(newAppointment);
